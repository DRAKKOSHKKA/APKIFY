...
`;
    content = content.replace(/#endif\s*\/\/\s*__cplusplus/, factoryCode + '#endif // __cplusplus');
  }
  fs.writeFileSync(headerPath, content, 'utf8');
  console.log('✓ Patched RuntimeScheduler.h (factory functions for Swift 6.1)');
}

// 3. Patch HostFunctionClosure.h
const hfcPath = path.resolve('node_modules/expo-modules-jsi/apple/Sources/ExpoModulesJSI-Cxx/include/HostFunctionClosure.h');
...
`;
  ...
  console.log('✓ Patched HostFunctionClosure.h (factory function for Swift 6.1)');
}
...
inline void appendPropNameId(HostObjectCallbacks::PropNameIds &vector, facebook::jsi::Runtime &runtime, const char *name) {
...
`;
    ...
    console.log('✓ Patched HostObjectCallbacks.h (appendPropNameId helper for move-only C++ types)');
  }
}
...
`;
  ...
  if (file.endsWith('JavaScriptRuntime.swift')) {
    ...
    content = content.replace(/expo\.RuntimeScheduler\(\)/g, 'createRuntimeScheduler()');
    content = content.replace(/expo\.RuntimeScheduler\((\w+),\s*(\w+)\)/g, 'createRuntimeSchedulerWithDispatch($1, $2)');
    content = content.replace(/expo\.HostFunctionClosure\((\w+),\s*(\w+),\s*(\w+)\)/g, 'createHostFunctionClosure($1, $2, $3)');
    ...
    //   }
    const propLoopOld = /for propertyName in propertyNames \{\s*let propNameId = facebook\.jsi\.PropNameID\.forUtf8\(iRuntime,\s*std\.string\(propertyName\)\)\s*vector\.push_back\(consuming:\s*propNameId\)\s*\}/;
    if (propLoopOld.test(content)) {
      ...
      content = content.replace(/let propNameId = facebook\.jsi\.PropNameID[^\n]+\n\s*/g, '');
    }
...
console.log(`✓ Patched ${swift62PatchCount} files with Swift 6.2 -> 6.1 compatibility fixes`);
...
if (fs.existsSync(scriptPath)) {
  ...
  if (!content.includes('CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY=""')) {
    content = content.replace(
