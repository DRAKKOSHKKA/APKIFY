...
// 2. Patch RuntimeScheduler.h — remove SWIFT_RETURNS_RETAINED and add factory functions
const headerPath = path.resolve(
	"node_modules/expo-modules-jsi/apple/Sources/ExpoModulesJSI-Cxx/include/RuntimeScheduler.h"
);
if (fs.existsSync(headerPath)) {
	let content = fs.readFileSync(headerPath, "utf8");
	// Remove SWIFT_RETURNS_RETAINED from constructors (if still present)
	content = content.replace(
		/SWIFT_RETURNS_RETAINED\s+RuntimeScheduler/g,
		"RuntimeScheduler"
	);

	// Add C++ factory functions BEFORE the #endif that Swift 6.1 can call
	// (Swift 6.1 cannot call constructors of SWIFT_SHARED_REFERENCE types directly,
	// but CAN call free functions that return pointers to them)
...
`;
	...
	console.warn(
		"⚠️ RuntimeScheduler.h not found at",
		headerPath
	);
}

// 2b. Patch HostFunctionClosure.h — add factory function
...
`;
		// Insert before the final #endif or at the end of the namespace
		// The file doesn't have #ifdef __cplusplus guards, so insert before the last line
	...
	console.warn(
		"⚠️ HostFunctionClosure.h not found at",
		hfcPath
	);
}
...
for (const file of swiftFiles) {
	...
	// 3d. Patch Task+immediate.swift — Task.immediate and Task(name:) don't exist in Swift 6.1
	if (file.endsWith("Task+immediate.swift")) {
		// Replace the entire polyfill body to avoid calling Task.immediate (Swift 6.2)
		// and Task(name:priority:operation:) which doesn't exist in Swift 6.1 either
...
`;
	...
	// 3e. Patch JavaScriptRuntime.swift — replace constructor calls with factory functions
	if (file.endsWith("JavaScriptRuntime.swift")) {
		// Replace expo.RuntimeScheduler() with createRuntimeScheduler()
		content = content.replace(
			/expo\.RuntimeScheduler\(\)/g,
			"createRuntimeScheduler()"
		);
		// Replace expo.RuntimeScheduler(scheduler, fn) with createRuntimeSchedulerWithDispatch(scheduler, fn)
		content = content.replace(
			/expo\.RuntimeScheduler\((\w+),\s*(\w+)\)/g,
			"createRuntimeSchedulerWithDispatch($1, $2)"
		);
		// Replace vector.push_back(consuming: x) with vector.push_back(x)
		// (The 'consuming:' label is a Swift 6.2 C++ interop feature)
		content = content.replace(
			/\.push_back\(consuming:\s*/g,
			".push_back("
		);
		// Replace expo.HostFunctionClosure(context, call, deallocate) with createHostFunctionClosure(context, call, deallocate)
		content = content.replace(
...
if (fs.existsSync(scriptPath)) {
	...
	console.warn(
		"⚠️ build-xcframework.sh not found at",
		scriptPath
	);
}
